...
static int __init phycore_am335_board_setup(char *str)
{
	...
	if (str) {
		...
		for(i = 0; i < ARRAY_SIZE(list_boards); i++) {
			if(strcmp(phycore_carrier_str, list_boards[i].name) == 0)
				valid_brd_name = 1;
		}
	}
	if(!valid_brd_name)
		printk("WARNING! No valid carrier Board found !!!\n");
...
static void __init phycore_am335_init(void)
{
	...
	{
		if(strcmp(phycore_carrier_str,list_devices[i].board_name) == 0)
